...
public class Cluster1Test {
    private static class Task implements Runnable {
        ...
        public Task(String serverName, Atomix atomix) {
    ...
    public void run1() {
        ...
        scheduledThreadPoolExecutor.scheduleAtFixedRate(new Task("server1", atomix), 0, 10, TimeUnit.SECONDS);
        try {
    ...
    public void run2() {
        try {
            ...
            atomix.start().join();
            ScheduledThreadPoolExecutor scheduledThreadPoolExecutor = new ScheduledThreadPoolExecutor(1);
            scheduledThreadPoolExecutor.scheduleAtFixedRate(new Task("server2", atomix), 0, 10, TimeUnit.SECONDS);
    ...
    public void run3() {
        ...
        atomix.start().join();
        ScheduledThreadPoolExecutor scheduledThreadPoolExecutor = new ScheduledThreadPoolExecutor(1);
        scheduledThreadPoolExecutor.scheduleAtFixedRate(new Task("server3", atomix), 0, 10, TimeUnit.SECONDS);
        try {
            waiter.await(1, TimeUnit.DAYS);
        } catch (TimeoutException e) {
            e.printStackTrace();
